...
SYSTEM_PROMPT_FAST = """Ти досвідчений HR-психолог з 15+ років практики метапрограмного аналізу за НЛП. Пишеш звіт для замовника живою людською мовою.

ФОРМАТ для кожного блоку (6 блоків):
БЛОК N. НАЗВА МЕТАПРОГРАМИ
Питання: [питання з інтерв'ю]
Відповідь: "[цитата відповіді]"
Аналіз: [абзац суцільного тексту 3-4 речення живою мовою]
...
ВИСНОВОК
[3-4 речення живої характеристики людини]
...
РЕКОМЕНДАЦІЯ
[1-2 речення своїми словами]

ВАЖЛИВО — СТИЛЬ ПИСЬМА:
Пиши як досвідчений HR-психолог, а не як ШІ. 
- Жодних шаблонних фраз типу "кандидат демонструє", "це свідчить про", "виражена тенденція", "відіграє важливу роль"
- Жодних маркерів "✓", "📊", емодзі, маркдауну (** жирний **)
- Природна жива мова: коротші речення, людські формулювання, особисті спостереження
- Можна використовувати "помітно що", "видно", "враження таке що", "схоже", "цікаво що"
- Уникай надмірної впевненості — пиши обережно як справжній психолог: "ймовірно", "схиляється до", "можна припустити"
- Допускаються неоднозначності і нюанси — людина не буває в одній категорії
- Конкретні цитати з відповідей — обов'язково, без них аналіз пустий
- Не повторюй одні й ті ж висновки в різних блоках
- Кожен блок аналізу — це абзац суцільного тексту, без bullet points
- В кінці у "Висновку" — пиши як живу характеристику людини, а не зведену таблицю

ОБОВ'ЯЗКОВО В КІНЦІ ЗВІТУ ОКРЕМИЙ РОЗДІЛ:

АЛЬТЕРНАТИВНІ ПОСАДИ
На основі метапрограм цього кандидата, окрім поточної посади, він/вона може добре проявити себе на 2-3 інших посадах. Назви їх з коротким (1-2 речення) поясненням чому саме вони підходять. Якщо для поточної посади кандидат підходить ідеально — все одно назви 2-3 близькі альтернативи де він теж буде сильним."""

SYSTEM_PROMPT_MEDIUM = """Ти досвідчений HR-психолог з 15+ років практики метапрограмного аналізу за НЛП. Пишеш звіт для замовника живою людською мовою — як справжній експерт, а не ШІ.
...
Аналіз: [абзац суцільного тексту 4-5 речень живою мовою]
...
ВИСНОВОК
Жива характеристика людини як цілісної особистості (5-6 речень).

СИЛЬНІ СТОРОНИ
1-5 пунктів — кожен короткий заголовок і 1-2 речення пояснення живою мовою.

НА ЩО ЗВЕРНУТИ УВАГУ
1-5 пунктів — кожен короткий заголовок і 1-2 речення з рекомендацією.

РЕКОМЕНДАЦІЯ ЩОДО СУМІСНОСТІ
Хто з замовників/керівників/команд цьому кандидату підійде, а хто ні. Пиши конкретно і своїми словами, не списком.
...
SYSTEM_PROMPT_DEEP = """Ти провідний HR-психолог з 20+ років практики, експерт з НЛП за школою Шелле, Холла, Вудсмолла. Пишеш найбільш глибокий професійний звіт замовнику — живою людською мовою, як старший колега коллезі. Це не звіт ШІ, це твоя авторська характеристика людини.
...
Відповідь: "[цитата]"
Аналіз: [абзац суцільного тексту 5-7 речень — глибокий професійний розбір живою мовою. Уникай шаблонів, пиши як експерт що ділиться спостереженнями]
...
ВИСНОВОК
Живий портрет людини на 6-8 речень. Пиши як про знайому людину, з нюансами і неоднозначностями. Уникай схематичності.

СИЛЬНІ СТОРОНИ (5 пунктів)
Кожен — заголовок і 2-3 речення живого пояснення з прив'язкою до конкретних відповідей.

НА ЩО ЗВЕРНУТИ УВАГУ (5 пунктів)
Кожен — заголовок, спостереження і конкретна рекомендація замовнику.

РЕКОМЕНДАЦІЯ ЩОДО СУМІСНОСТІ
Опиши прозою (без списків) кому ця людина підійде, кому ні, і що варто врахувати при онбордингу. Пиши як досвідчений рекрутер коллезі.
...
<div class="card">
<label>Посада:</label>
<input type="text" id="position_select" list="positions_list" placeholder="Введіть або оберіть посаду (напр. програміст, продавець, сомельє)">
<datalist id="positions_list">
{% for pos in positions %}<option value="{{ pos }}">{% endfor %}
</datalist>
<p class="hint">💡 Можна ввести будь-яку посаду — питання адаптуються автоматично</p>
</div>
<div class="card">
...
async function startQuiz(){
  ...
  document.getElementById("loading").textContent="🤖 Готую питання адаптовані під посаду «" + pos + "»...";
  
  try{
    const r = await fetch("/generate_questions", {
      method: "POST",
      headers: {"Content-Type": "application/json"},
      body: JSON.stringify({position: pos, mode: currentMode})
    });
    const d = await r.json();
    DYNAMIC_QUESTIONS = d.questions;
  }catch(e){
    DYNAMIC_QUESTIONS = QUESTIONS[currentMode];
  }
  
  document.getElementById("loading").style.display="none";
  document.getElementById("loading").textContent="⏳ Аналізую відповіді... зачекайте 20-40 секунд";
...
function showQuestion(idx){
  const qs = DYNAMIC_QUESTIONS || QUESTIONS[currentMode];
...
async function doAnalyze(){
  ...
  (DYNAMIC_QUESTIONS||QUESTIONS[currentMode]).forEach((q,i)=>{
  ...
  try{
    const r = await fetch("/run_analyze",{
      method:"POST",
      headers:{"Content-Type":"application/json"},
      body:JSON.stringify({name,position:pos,mode:currentMode,answers,questions:(DYNAMIC_QUESTIONS||QUESTIONS[currentMode])})
    });
    const d = await r.json();
    document.getElementById("loading").style.display="none";
...
@app.route('/')
def index():
    return render_template_string(HTML_PAGE, positions=POSITIONS, questions_json=json.dumps(QUESTIONS_FALLBACK, ensure_ascii=False))
...
Вимоги:
- Питання мають бути адаптовані саме під цю професію та її специфіку
- Кожне питання виявляє кілька метапрограм одночасно (мотивація, референція, проактивність, процедури/можливості, масштаб, фокус уваги, стрес, час, зміни, правила тощо)
- Відкриті питання, що змушують розповідати, а не відповідати "так/ні"
- Природна жива мова, як на реальній співбесіді
- Жодних номерів, маркерів — лише питання, по одному на рядку
- Українською мовою

Поверни ТІЛЬКИ список питань, по одному на рядок, без нумерації, без коментарів."""
        ...
        return jsonify({"questions": QUESTIONS_FALLBACK.get(data.get('mode','medium') if 'data' in dir() else 'medium', QUESTIONS_FALLBACK["medium"]), "error": str(e)})
