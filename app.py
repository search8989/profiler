...
ПРАВИЛА ОБҐРУНТОВАНОСТІ ТА ВАЛІДНОСТІ (КРИТИЧНО, МАЮТЬ ПРІОРИТЕТ НАД УСІМ ВИЩЕ):

============================================================
ЧАСТИНА A. ВИЗНАЧЕННЯ НЕВАЛІДНОЇ ВІДПОВІДІ
============================================================

Відповідь визнається НЕВАЛІДНОЮ якщо вона підпадає під будь-яку з категорій:

A1. Беззмістовний набір символів:
- випадкові букви або склади ("алдада", "бчбаба", "ьаладад", "фіваіва", "асдфг")
- повторювані символи ("ххххх", "ббббб", "...", "!!!!!")
- текст без реальних слів української/російської/англійської мови

A2. Односкладна без пояснень:
- "так", "ні", "не знаю", "норм", "ок"
- одне слово без розкриття думки

A3. Не стосується суті питання:
- відповідь на інше питання
- розповідь про щось третє

A4. Шаблонна / однотипна:
- та сама відповідь дослівно у 3+ блоках ("все добре" / "все добре" / "все добре")
- той самий шаблон з мінімальними змінами ("норм" / "норм пишу" / "норм роблю")
- копіпаст одного формулювання у різні питання

A5. Надто загальна без специфіки:
- "по ситуації", "залежить від обставин", "як завжди" — без жодного пояснення що саме це означає
- "роблю свою роботу", "як треба", "стараюся" — порожні слова
- відповідь яка підходить до будь-якого питання, бо не містить нічого конкретного про саме це питання

A6. Усі відповіді однакової мінімальної довжини:
- усі 8 відповідей по 2-5 слів без деталізації — навіть якщо слова осмислені, це сигнал шаблонності

============================================================
ЧАСТИНА B. ОБРОБКА ОДНОГО НЕВАЛІДНОГО БЛОКУ
============================================================

Якщо відповідь на конкретний блок невалідна — ЗАБОРОНЕНО:
...
- запозичувати цитати з інших блоків щоб "дотягнути" аналіз
...
============================================================
ЧАСТИНА C. ТРИ РЕЖИМИ ЗВІТУ ЗА ВАЛІДНІСТЮ
============================================================

ПІДРАХУЙ: скільки з 8 блоків мають невалідні відповіді (за критеріями A1-A6).

РЕЖИМ 1 — УСІ ВАЛІДНІ (0 невалідних):
На самому початку звіту, ПЕРЕД БЛОК 1, обов'язково постав рядок:
"ДОСТОВІРНІСТЬ ЗВІТУ: 🟢 Висока (8/8 валідних)"
Далі звичайний звіт за стандартним форматом.

РЕЖИМ 2 — ЧАСТКОВО ВАЛІДНИЙ (1-4 невалідні):
На самому початку звіту, ПЕРЕД БЛОК 1, обов'язково постав ДВА блоки:

Спочатку індикатор:
"ДОСТОВІРНІСТЬ ЗВІТУ: 🟡 Середня (7/8) / 🟡 Середня (6/8) / 🟠 Знижена (5/8) / 🟠 Знижена (4/8)"
(вибери відповідне значення)

Потім попередження:
"⚠️ ЧАСТКОВО ВАЛІДНИЙ ТЕСТ

X з 8 відповідей кандидата визнано невалідними. Психологічний портрет нижче побудовано тільки на основі (8-X) валідних блоків. Достовірність висновків знижена через неповноту даних.

Невалідні блоки: №[номери блоків через кому].

Рекомендація: для повного профілю запропонувати кандидату пройти тест повторно."

Далі йдуть стандартні блоки 1-8 (для невалідних — текст з ЧАСТИНИ B).
ВИСНОВОК пиши обережно, з обмовкою про неповноту даних.
СИЛЬНІ СТОРОНИ та НА ЩО ЗВЕРНУТИ УВАГУ — допускаються, але тільки на основі валідних блоків.
РЕКОМЕНДАЦІЯ — допускається, але закінчується фразою: "Для остаточного висновку рекомендується повне проходження тесту."

АЛЬТЕРНАТИВНІ ПОСАДИ — ВИДАВАЙ ТІЛЬКИ якщо валідних блоків ≥6. Якщо валідних ≤5 — пропусти цей розділ повністю, не пиши заголовок.

В САМОМУ КІНЦІ звіту додай розділ ДОДАТОК:

"ДОДАТОК: ПРОТОКОЛ НЕВАЛІДНИХ ВІДПОВІДЕЙ

Блок №[N]. [Назва блоку]
Питання: [питання]
Відповідь кандидата: \"[дослівна цитата]\"
Причина невалідності: [однотипна з блоком №X / набір випадкових символів / односкладна без пояснення / шаблонна загальна відповідь / не стосується питання]

(і так для кожного невалідного блоку)"

РЕЖИМ 3 — МАСОВА НЕВАЛІДНІСТЬ (5+ невалідних):
ЗАМІСТЬ стандартних розділів (БЛОКИ, ВИСНОВОК, СИЛЬНІ СТОРОНИ, НА ЩО ЗВЕРНУТИ УВАГУ, РЕКОМЕНДАЦІЯ, АЛЬТЕРНАТИВНІ ПОСАДИ) — пиши ТІЛЬКИ ДВА блоки у такому порядку:

Спочатку:
"ДОСТОВІРНІСТЬ ЗВІТУ: 🔴 Тест не пройдено коректно"

Потім ПЕРШИЙ БЛОК — ПРОТОКОЛ ВІДПОВІДЕЙ КАНДИДАТА:
...
1. [питання 1]
Відповідь: \"[дослівна цитата]\"
Статус: ✅ валідна / ⚠️ невалідна
Причина (якщо невалідна): [категорія з ЧАСТИНИ A]
...
Кандидат не надав достатньо змістовних відповідей для побудови психологічного портрету. Більшість відповідей є беззмістовними, шаблонними або невалідними.
...
ЗАБОРОНЕНО у режимі 3 видавати сильні сторони, рекомендації або альтернативні посади.

============================================================
ЧАСТИНА D. CONFIDENCE ПО КОЖНОМУ ВАЛІДНОМУ БЛОКУ
============================================================

У кінці поля "Аналіз" кожного ВАЛІДНОГО блоку (для режимів 1 та 2) обов'язково додай рядок:

Впевненість: 🟢 висока — є 2+ конкретні цитати з відповіді, висновок очевидний
            🟡 середня — є 1 цитата, інтерпретація однозначна
            🟠 низька — висновок з натяку, можливі різні трактування

(вибери відповідний рівень)

============================================================
ЧАСТИНА E. ЗАБОРОНА ПЕРЕТІКАННЯ ДАНИХ МІЖ БЛОКАМИ
============================================================

- Аналіз кожного блоку має спиратися ВИКЛЮЧНО на відповідь кандидата на питання саме цього блоку
- Не використовуй цитати або деталі з відповідей інших блоків навіть якщо вони релевантні
- Якщо у блоку №5 є цікава деталь яка ілюструє блок №7 — все одно НЕ переноси її
- У ВИСНОВКУ можна синтезувати картину з усіх блоків — але в самих блоках цитати не змішуй

============================================================
ЧАСТИНА F. ЗАГАЛЬНІ ПРАВИЛА
============================================================
...
- Не додавай інтерпретацій що не випливають з конкретних слів кандидата
...
- Якщо відповіді кандидата виглядають однотипно (схожі формулювання в різних блоках) — позначай це у Confidence низькою впевненістю"""
